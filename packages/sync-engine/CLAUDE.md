...
**Storage is not serialized across realms.** `settingsBinding.writeOne` read-modify-writes the whole settings blob, and so does the page realm's `settings-store`; the two live in different realms, so no in-memory queue can order them. See ENG-82 (per-key settings storage) and ENG-83 (a `KeyValueStore` change subscription).
