...
**A disable cancels the cycle in flight.** `disableSync()` bumps an account epoch, and `runCycle` hands the cycle an `isCancelled` closure over it (`CycleDeps.isCancelled`), so `pullOnce` stops between pages and between records and `pushOnce` between batches. A cancelled pull saves nothing — a cursor must never outlive the account that earned it, or a later enable's first `getChanges` silently skips every record below that high-water mark. `syncNow()` therefore answers `SyncNowResult`, which is `SyncOutcome` plus `{kind:'cancelled'}`; `cancelled` is deliberately outside `SyncOutcome` so the compiler stops it being recorded, persisted or painted as a cycle that ran. Belt and braces on the same fact: `enableSync` also resets the cursor when it enables a device that was off, since disable's metadata reset is best-effort (a re-auth on a still-enabled device keeps its cursor).
