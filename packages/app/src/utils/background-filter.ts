...
export function getBackgroundFilterStyle(dim: number, blur: number): CSSProperties {
  ...
  if (clampedBlur > 0) {
    ...
    // Consumers must be position:fixed, live inside a fixed subtree, or sit under a
    // clipping (overflow-hidden) parent — otherwise the bleed extends the nearest
    // scroll container's scrollable area.
