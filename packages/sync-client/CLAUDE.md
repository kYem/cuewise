...
A record's optional `baseSeq` makes its push conditional on the server row still being at that seq, or gone (`0` means gone only); refused rows come back under `conflicts` with the current row. An older server's bare `{cursor}` is normalised to every record under `applied` (no `seq`) and no `conflicts`, with one warning per client; a reply carrying only one of the two arrays is an `invalid_response`.
