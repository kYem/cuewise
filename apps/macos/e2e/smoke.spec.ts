...
test('reused extension UI renders on every surface without errors (WebKit)', async ({ page }) => {
  ...
  // Scoped to the dialog: `exact` alone is not enough, since ConceptToolbar's dock button
  // carries an aria-label of exactly "Next" whenever a concept card is the due surface.
