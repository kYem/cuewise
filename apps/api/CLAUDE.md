...
| `tokens` | `token_hash` (PK), `id` (unique where non-empty), `user_id`, `device_name`, `expires_at`, `revoked_at`, `last_used_at`, `window_start`/`window_count`, `created_at` | The raw session token is never stored, only its SHA-256. Rate-limit counters live on the token's own row, so per-token limiting needs no separate table. `id` is an opaque public handle used by `/v1/sessions` — deliberately not `token_hash`, so the auth lookup key never reaches a URL or an invocation log. Holds no IP, geolocation or user-agent, by design. |
...
| `GET` | `/v1/sessions` | List the caller's live sessions (`id`, `deviceName`, `createdAt`, `lastUsedAt`, `current`). Revoked, expired, and un-migrated (`id = ''`) rows are omitted. | Yes |
