...
| `GET` | `/v1/integrations/notion/tables` | The data sources shared with the integration, for the user to pick from | Yes |
...
| `PATCH` | `/v1/integrations/notion/items/:pageId` | Write completion only, `{done}`. 422 `provider_todo_group_missing` for `done: false` on a status with no `To-do` group; 404 `not_found` when that page is gone or trashed; 403 `provider_write_forbidden` when Notion refuses the write for this token (the table stays picked either way) | Yes |
| `DELETE` | `/v1/integrations/notion` | Take the stored grant (one `DELETE … RETURNING`) and revoke it upstream, best-effort — works even if the token no longer decrypts. `DELETE /v1/account` does the same revocation before the row goes with the user | Yes |
...
| `internal` | 500 | Unhandled exception, upstream (JWKS) outage, or a config fault: empty signing key / client-id, a `PROVIDER_TOKEN_KEY` that is not 32 bytes, Notion rejecting our client secret, a 403/404 on an endpoint that names no user resource (`/search`, `/oauth/*` — integration capabilities or our URL), or any other Notion 4xx — a request only we could have malformed (`missing_version`, `invalid_json`). 429 and 409 `conflict_error` are Notion's retryable 4xx and answer 503; so does `validation_error`, by our policy, since a property renamed under a write causes it (a write to a trashed page also answers 400, and is looked up once so it reads as 404 `not_found` instead) |
