...
| `PATCH` | `/v1/integrations/notion/items/:pageId` | Write completion only, `{done}`. 422 `provider_todo_group_missing` for `done: false` on a status with no `To-do` group; 404 `not_found` when that page is gone (the table stays picked) | Yes |
| `DELETE` | `/v1/integrations/notion` | Revoke upstream (best-effort) and drop the stored grant — works even if the token no longer decrypts. `DELETE /v1/account` does the same revocation before the row goes with the user | Yes |
...
| `internal` | 500 | Unhandled exception, upstream (JWKS) outage, or a config fault: empty signing key / client-id, a `PROVIDER_TOKEN_KEY` that is not 32 bytes, Notion rejecting our client secret, a 403/404 on an endpoint that names no user resource (`/search`, `/oauth/*` — integration capabilities or our URL), a 403 on a page write (update capability), or any other Notion 4xx — a request only we could have malformed (`missing_version`, `invalid_json`). 429 and 409 `conflict_error` are Notion's retryable 4xx and answer 503; so does `validation_error`, by our policy, since a property renamed under a write is the one 4xx a user causes |
