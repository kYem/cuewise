...
| `internal` | 500 | Unhandled exception, upstream (JWKS) outage, or a config fault: empty signing key / client-id, a `PROVIDER_TOKEN_KEY` that is not 32 bytes, Notion rejecting our client secret, a 403 on Notion's `/search` (integration capabilities, not a table the user un-shared), or any other Notion 4xx — a request only we could have malformed (`Notion-Version`, `invalid_json`); 429, 409 `conflict_error` and `validation_error` are Notion's retryable 4xx and answer 503 instead |
