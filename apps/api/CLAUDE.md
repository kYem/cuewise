...
| `provider_tokens` | `(user_id, provider)` (PK), `ciphertext`, `iv`, `refresh_ciphertext`, `refresh_iv`, `workspace`, `data_source_id`, `created_at` | **The only table holding a credential the server itself decrypts** — a third-party token is useless to us wrapped in a client-only key. AES-GCM under `PROVIDER_TOKEN_KEY` (a Worker secret, deliberately *not* the user's sync key). The refresh pair is nullable: Notion issues a refresh token only for grants that expire. Three writers, each touching only its own columns — `putProviderGrant` (connect; keeps `data_source_id` in SQL), `updateProviderTokens` (renewal), `setProviderDataSource` (picker) — and no whole-row replace, so a renewal and a selection in flight together cannot clobber each other. `data_source_id` is the table queried; Notion's schema lives on the data source, not the database. |
...
| `POST` | `/v1/integrations/notion/claim` | `{code, codeVerifier}`. Consumes the parked code and stores the grant under the **calling session** — the only step that binds a grant to an account, so it lands on whoever authorised, never on whoever minted the link | Yes |
...
| `GET` | `/v1/integrations/notion/items` | The mirrored rows as `{pageId, text, done}` plus `truncated` (the 500-row bound was hit). No Notion content is persisted — this proxies | Yes |
...
| `internal` | 500 | Unhandled exception, upstream (JWKS) outage, or a config fault: empty signing key / client-id, a `PROVIDER_TOKEN_KEY` that is not 32 bytes, Notion rejecting our client secret, or a 403 on Notion's `/search` (integration capabilities, not a table the user un-shared) |
...
6. **Schema changes are additive migrations.** A new table/column is a new numbered file in `migrations/` — the test harness and `wrangler d1 migrations apply` both apply every migration in order, so there's no "edit an old migration" path once one has reached production (a local dev DB that applied a since-squashed file keeps its stale `d1_migrations` row harmlessly).
