...
| `PATCH` | `/v1/integrations/notion/items/:pageId` | Write completion only, `{done}`. 422 `provider_todo_group_missing` for `done: false` on a status with no `To-do` group; 404 `not_found` when that page is gone or trashed; 403 `provider_write_forbidden` when the user cannot edit it (the table stays picked either way) | Yes |
...
| `provider_write_forbidden` | 403 | Notion refused the page write for the user: the token acts as them, so a table they can read but not edit answers this, at warn. The table stays picked |
