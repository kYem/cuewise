...
| `pairings` | `id` (PK), `user_id`, `requester_session_id`, `requester_public_key`, `approver_session_id`, `approver_public_key`, `envelope`, `created_at`, `expires_at` | ENG-50 transient device-pairing relay rows — 10 min TTL, one live request per requester session. `requester_public_key`/`approver_public_key`/`envelope` are opaque, purged by the daily cron alongside tombstones. |
