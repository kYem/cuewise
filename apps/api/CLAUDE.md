...
```
worker entry (src/worker.ts) — fetch + scheduled() daily purges (tombstones, pairings, unclaimed Notion grants)
...
| `auth_codes` | `code_hash` (PK), `payload` (JSON), `expires_at`, `used_at`, `code_challenge` | The server-bounce one-time exchange codes (Apple and Google sign-in, and a parked Notion grant — `payload.provider` records which). 60s TTL; `code_challenge` binds it to a PKCE verifier. Expired sign-in codes are swept on the next mint; the daily cron purges them too (`purgeExpiredSignInCodes`). An expired parked Notion grant waits for the cron's `revokeExpiredParkedGrants`, which deletes the row only once Notion has accepted the revoke (each run walks the backlog oldest first within a budget of 40 grants, one revoke each, moving past a row it could not revoke; a failed revoke is retried daily, abandoned loudly after 7 days). `consumeAuthCode` DELETEs the row (single-use + PII gone at once), so `used_at` is now vestigial. |
...
| `provider_tokens` | `(user_id, provider)` (PK), `ciphertext`, `iv`, `refresh_ciphertext`, `refresh_iv`, `workspace`, `data_source_id`, `renewal_started_at`, `created_at` | **The only table holding a credential the server itself decrypts** — a third-party token is useless to us wrapped in a client-only key. AES-GCM under `PROVIDER_TOKEN_KEY` (a Worker secret, deliberately *not* the user's sync key). The refresh pair is nullable: Notion declares `refresh_token` as `string | null` with no `expires_in`, and we treat a null one as a grant that does not expire. No whole-row writer: the token writers (`putProviderGrant` on connect, `updateProviderTokens` on renewal) never touch `data_source_id` — `putProviderGrant` keeps it in SQL — and the picker (`setProviderDataSource`) touches nothing else, so a renewal and a selection in flight together cannot clobber each other. `renewal_started_at` is a 30s renewal claim keyed on the ciphertext the request opened, and the renewal's write is keyed the same way — Notion rotates refresh tokens, so a concurrent, stale or reconnect-crossed refresh must never store over the row (see the `SyncStore` port comments). A disconnect takes the row in one `DELETE … RETURNING` and revokes the access token, which is the whole grant: measured 2026-09-25, `/oauth/revoke` ignores a refresh token (200, nothing revoked) and an access-token revoke kills the paired refresh token too. `deleteUser` returns the rows it removed so account deletion revokes them the same way; an expired, unclaimed park is revoked by the daily cron. `data_source_id` is the table queried; Notion's schema lives on the data source, not the database. |
...
| `POST` | `/v1/changes` | Batch push, ≤100 records, ≤64 KB ciphertext/record. A record may carry `baseSeq`, the seq the device last saw: it lands only if the row is still at that seq, or absent (`0` matches no row, so it lands only where none exists). Answers `{cursor, applied, conflicts}`; conflicts are the current rows. Without `baseSeq` a record upserts unconditionally (legacy clients). | Yes |
...
| `DELETE` | `/v1/account` | Delete user, identities, tokens, records, key envelopes, and any third-party provider grants | Yes |
...
| `GET` | `/v1/integrations/notion/start` | Begin connecting Notion: `return_uri` + `code_challenge`, answers `{authorizeUrl}` as JSON so the app opens it. The state names **no account** | Yes |
| `GET` | `/v1/integrations/notion/callback` | Notion's redirect target. Exchanges the code server-side and **parks** the encrypted grant behind a one-time PKCE-bound code on the `cuewise://` deep link — stores nothing against any account | No |
| `POST` | `/v1/integrations/notion/claim` | `{code, codeVerifier}`. Consumes the parked code and stores the grant under the **calling session** — the only step that binds a grant to an account, so it lands on whoever authorised, never on whoever minted the link. A reconnect revokes the grant it replaced — Notion keeps it live otherwise — after storing the new one, and without the response waiting on it | Yes |
| `GET` | `/v1/integrations/notion/tables` | The data sources shared with the integration, for the user to pick from, as `{workspace, tables, truncated}` — `truncated` when the page bound stopped the search before Notion ran out | Yes |
| `PUT` | `/v1/integrations/notion/selection` | Validate and remember the picked table, `{dataSourceId}`. 422 unless it has a status with a `Complete` group or (with no status property at all) a `Done` checkbox | Yes |
| `GET` | `/v1/integrations/notion/items` | The mirrored rows as `{pageId, text, done}` plus `truncated` (the read stopped before the table ended: the 500-row bound, or a `has_more` Notion gave no cursor for). No Notion content is persisted — this proxies | Yes |
| `PATCH` | `/v1/integrations/notion/items/:pageId` | Write completion only, `{done}`. 422 `provider_todo_group_missing` for `done: false` on a status with no `To-do` group; 404 `not_found` when that page is gone or trashed, or is not a row of the selected table (checked before writing, so a list read before a table switch cannot complete a task elsewhere); 403 `provider_write_forbidden` when Notion refuses the write for this token (the table stays picked either way) | Yes |
| `DELETE` | `/v1/integrations/notion` | Take the stored grant (one `DELETE … RETURNING`) and revoke it upstream, best-effort — works even if the token no longer decrypts. `DELETE /v1/account` revokes the grants `deleteUser` removed, after the delete, so a grant claimed mid-delete is still revoked | Yes |
...
**Server bounce (macOS deep-link sign-in)**: mirrors the Apple flow below (`routes/google.ts`, shared helpers in `routes/bounce-shared.ts`) with two divergences. First, Google's server flow needs a confidential-client **authorization-code exchange** at the callback — `GoogleCodeExchanger` POSTs the code + `GOOGLE_OAUTH_CLIENT_ID` + `GOOGLE_CLIENT_SECRET` (a Worker secret) to Google's token endpoint and gets the id_token back (Apple's `response_type=code id_token` avoids this). The exchanger classifies failures: `invalid_client`/`unauthorized_client`/`redirect_uri_mismatch` are loud config faults, `invalid_grant` is a bad code. Second, once the callback's `state` verifies (HMAC + allowlist), **both success and failure return a 200 HTML interstitial** (a status message + "You can close this tab.") that fires the `cuewise://` deep link via script + meta-refresh, carrying either `?code=` or a sanitized `?error=` (`access_denied` | `auth_failed` | `server_error`) — a bare 302 leaves the tab looking "stuck on Google", and problem+json would strand the native app until its callback timeout. The page is `Cache-Control: no-store` (it embeds the one-time code). The bounced code is then redeemed at `POST /v1/auth/token` with `{provider: 'google', credential: <code>, codeVerifier, deviceName}` — `codeVerifier` present ⇒ bounced-code redemption (`redeemBouncedCode`), absent ⇒ id-token verification. Redemption cross-checks the request's provider against the minted payload's provider.
...
`createSession` mints 32 random bytes, returns the raw token once, and stores only its SHA-256 (`tokens.token_hash`). The raw token and its hash are branded types (`RawSessionToken`/`SessionTokenHash` in `crypto-utils.ts`), so passing one where the other belongs — e.g. a hash into `revokeSession` — is a compile error, not a silent no-op. `SyncStore`'s pairing methods are branded the same way, from `@cuewise/crypto` (`PairingCommitment`/`PairingPublicKeyB64`/`PairingNonceB64`/`PeerWrappedEnvelope`) — the server only relays these, so `routes/pairings.ts` asserts each brand beside the validator that bounds it, and `d1-store.ts`'s row types assert them again on the way back out of D1. Expiry is sliding: every successful `lookupSession` pushes `expires_at` forward another `SESSION_TTL_MS` (90 days) and updates `last_used_at`. `POST /v1/auth/logout` sets `revoked_at`, which `lookupSession` checks going forward.
...
| `provider_reauth_required` | 409 | The third-party grant died. Not 401: clients treat every 401 as a lost Cuewise session and sign out. Distinct from `invalid_token`, which is about the caller's own session — here the session is fine and only the provider connection needs redoing. The row is dropped when Notion itself refused the token, but **kept** when it merely fails to decrypt (a rotated or mis-deployed `PROVIDER_TOKEN_KEY`): putting the key back must restore every grant, and a reconnect replaces the row anyway |
| `provider_claim_invalid` | 400 | `/claim` got an unknown, expired, replayed or sign-in code, or a wrong verifier — restart the connect. The caller's session is fine |
| `provider_schema_unusable` | 422 | The chosen table has no usable completion property: a status property whose groups do not match refuses outright (a renamed or localized `Complete` group must not silently fall through to a checkbox). Also answered at read and write time, so drift prompts instead of silently never completing anything |
| `provider_write_forbidden` | 403 | Notion refused the page write for this token (`restricted_resource`: a permission it lacks, or a workspace block limit), at warn with the reason. The table stays picked |
...
| Per-token, fixed window (`rate-limit.ts`) | `/v1/integrations/notion`, `.../start`, `.../claim`, `.../tables`, `.../selection`, `.../items/*` | 60 req | 60s — shares the per-token budget above |
| Per-IP, fixed window, isolate-local (`ip-rate-limit.ts`) | `/v1/auth/token`, `/v1/auth/{apple,google}/start`, `/v1/auth/{apple,google}/callback`, `/v1/integrations/notion/callback` | 30 req (default) | 60s — in-memory `Map`, resets on isolate recycle; defense-in-depth, production also fronts these with WAF rules |
...
# also fill in GOOGLE_CLIENT_IDS / GOOGLE_OAUTH_CLIENT_ID / APPLE_CLIENT_ID / NOTION_CLIENT_ID /
# PUBLIC_BASE_URL / ALLOWED_RETURN_URIS as plain `vars` in wrangler.jsonc
npx wrangler deploy
...
3. **Empty `STATE_SIGNING_KEY` ⇒ the bounce routes fail closed**, by design (`requireStateSigningKey`): Apple and Notion answer 500; Google's `/start` relays `?error=server_error` to the already-allowlisted return URI and its `/callback` answers 500. That's not a bug to work around — provision the secret (and `GOOGLE_CLIENT_SECRET` / `GOOGLE_OAUTH_CLIENT_ID` for Google, `NOTION_CLIENT_SECRET` + a 32-byte `PROVIDER_TOKEN_KEY` for Notion). The Notion routes that *use* a grant (`tables`, `selection`, `items`, `PATCH`) validate the key's length before reading it, so a truncated secret 500s instead of making stored grants look revoked; the two delete paths (`DELETE /notion`, `DELETE /account`) run the same check but never fail on it — a bad key must never trap a disconnect — and log the skipped revocation at error, naming the user.
