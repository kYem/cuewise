...
| `POST` | `/v1/integrations/notion/claim` | `{code, codeVerifier}`. Consumes the parked code and stores the grant under the **calling session** — the only step that binds a grant to an account, so it lands on whoever authorised, never on whoever minted the link. A reconnect overwrites the previous grant without revoking it upstream (see the route comment) | Yes |
