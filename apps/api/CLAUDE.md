...
| `provider_tokens` | `(user_id, provider)` (PK), `ciphertext`, `iv`, `refresh_ciphertext`, `refresh_iv`, `workspace`, `data_source_id`, `created_at` (`database_id` exists from 0009 but is unused) | **The only table holding a credential the server itself decrypts** — a third-party token is useless to us wrapped in a client-only key. AES-GCM under `PROVIDER_TOKEN_KEY` (a Worker secret, deliberately *not* the user's sync key). The refresh pair is nullable: Notion issues a refresh token only for grants that expire. Routes write it through `updateProviderTokens` / `setProviderDataSource`, never a whole-row replace — two requests renewing and selecting at once must not clobber each other. `data_source_id` is the table queried; Notion's schema lives on the data source, not the database. |
...
| `DELETE` | `/v1/integrations/notion` | Revoke upstream (best-effort) and drop the stored grant — works even if the token no longer decrypts | Yes |
...
4. **The `state` blob is trustworthy only because it's HMAC-signed.** Anything you add to `BounceState` (`routes/bounce-shared.ts`, shared by Apple, Google and Notion) inherits that guarantee automatically; anything you read from an unsigned source (a query param, a header) doesn't, no matter how it's later combined with `state`. **That guarantee is integrity, not identity**: the signature proves *we minted it*, never *who is completing the flow*. Never put an account id in a state — the first Notion draft did, and a leaked `/start` link would have grafted a victim's grant onto the attacker's account. Bind identity at redemption, with a session, as every bounce does.
