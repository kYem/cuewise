import { formatDate, type Goal, type ObjectiveProgress } from '@cuewise/shared';
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p
            className={`text-sm font-medium truncate ${objective.completed ? 'text-secondary line-through' : 'text-primary'}`}
          >
            {objective.text}
          </p>
            ...
            <div className="flex-1 h-1.5 bg-muted rounded-full overflow-hidden">
              <div
                className={`h-full transition-all ${
                  objective.completed ? 'bg-green-500' : 'bg-primary-500'
                }`}
                style={{ width: `${percent}%` }}
              />
            </div>
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h3
            className={`font-medium ${objective.completed ? 'text-secondary line-through' : 'text-primary'}`}
          >
            {objective.text}
          </h3>

          {objective.description && (
            <p className="text-sm text-secondary mt-1 line-clamp-2">{objective.description}</p>
          )}
          ...
          {daysLabel && (
            <div className="flex items-center gap-1.5 mt-3">
              ...
              >
                {daysLabel} ({formatDate(objective.date)})
              </span>
            </div>
          )}
        </div>
